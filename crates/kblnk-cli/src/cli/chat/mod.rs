...
impl ChatContext {
    ...
    async fn handle_response(
        ...
        let mut chat_response_parser = ChatResponseParser::new(response_stream);
        
        loop {
            match chat_response_parser.next().await {
                Ok(responses) =>{
                    for response in responses {
                        match response {
                            ChatResponse::Response(response_message) => {
                                execute!(
                                self.output,
                                style::Print(response_message),
                                style::Print("\n"),
                            )?;
                            }
                            ChatResponse::Code(code_message) => {
                                todo!("Code formatted responses are not implemented!")
                            }
                            ChatResponse::EndStream { message } => {
                                self.conversation_state.push_llm_message(message);
